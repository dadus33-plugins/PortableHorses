...
public class PHCommand implements CommandExecutor, TabCompleter {
    ...
    final String cm;
    final net.nordicraft.phorses.PortableHorses plugin;
    final NMSHandler handler;
    Storage c;
    public PHCommand(Storage c, net.nordicraft.phorses.PortableHorses p, NMSHandler handler){
        if(c.ENABLE_HELP_COMMAND)
            tabs = Arrays.asList("help", "givesaddle", "reload");
        else
            tabs = Arrays.asList("givesaddle", "reload");
        this.c = c;
        this.plugin = p;
        ...
        if(plugin.getCommand("ph").getPermissionMessage()!=null)
            ...
            cm = ChatColor.RED+"I\\'m sorry, but you do not have permission to perform this command. Please contact the server administrators if you believe that this is in error.";
    }
